...
import { Layers, Map, CloudSun, ShieldAlert, Car, Database } from "lucide-react";
...
export const MapLayerPanel = ({ activeLayers, toggleLayer }) => {
    ...
    return (
        <div className="absolute top-3 left-3 w-64 bg-[var(--color-surface-primary)] border border-[var(--color-border)] rounded shadow-md z-[1000] overflow-hidden pointer-events-auto">
            <div className="bg-[var(--color-surface-secondary)] p-2.5 flex items-center justify-between border-b border-[var(--color-border)]">
            ...
            <div className="max-h-[60vh] overflow-y-auto p-2 custom-scrollbar space-y-3">
                {groups.map((g, i) => (
                    <div key={i} className="space-y-1">
                        <div className="flex items-center gap-1.5 px-2 py-1 text-[10px] text-[var(--color-text-secondary)] font-bold uppercase tracking-wider">
                            {g.icon} {g.name}
                        </div>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-2">
                                            <div className={`w-3.5 h-3.5 rounded-sm flex items-center justify-center border transition-colors ${isActive ? 'bg-blue-600 border-blue-600' : 'border-slate-300 bg-white'}`}>
                                        ...
                                        {item.badge && (
                                            <span className={`text-[9px] px-1.5 py-0.5 rounded font-mono font-bold tracking-tight ${
                                                item.badge === 'LIVE' ? 'bg-emerald-100 text-emerald-800' :
                                                item.badge === 'ESTIMATE' ? 'bg-slate-100 text-slate-600' :
                                                item.badge === 'DERIVED' ? 'bg-amber-100 text-amber-800' :
